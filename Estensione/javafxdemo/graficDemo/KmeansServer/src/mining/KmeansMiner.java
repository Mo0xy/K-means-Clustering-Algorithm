...
public class KmeansMiner {
    ...
    public KmeansMiner(String fileName) throws FileNotFoundException, IOException, ClassNotFoundException{
        try{
            ...
            for (int i = 0; i < 3; i++){
                dbParameters[i] = (String) inStream.readObject();
            }
            C  = (ClusterSet) inStream.readObject();
            inStream.close();
            inFile.close();

            DBMetaData.getDBMetaDataInstance().updateDBParameters(
                    dbParameters[0],
                    dbParameters[1],
                    dbParameters[2]);
    ...
    /**
     * Salva su file il clusterset con i parametri del database.
     *
     * @param fileName percorso del file;
     * @param dbParameters parametri del db;
     */
    public void save(String fileName, String[] dbParameters) {
        try {
            ObjectOutputStream out = new ObjectOutputStream(new FileOutputStream(fileName));
            for (String s : dbParameters){
                out.writeObject(s);
            }
            out.writeObject(this.C);
            out.close();
